...
try :
	...
	pk.dump({1000:{"name":'','mnum':'','pin':'','city':'','balance':0}},dat_file)
	dat_file.close()
...
def login():
	...
	def login_page():
		dat_file = open("acc_data.dat","rb")
		dat_dict = pk.load(dat_file)
		acc_dict = dat_dict[acc_no]

		u_name,u_nmum,u_pin,u_city,u_balance = list(acc_dict.values())
	...
	if e_accno.get().isdigit():
		if int(e_accno.get()) in dat_dict.keys() and e_pinno.get() != '':
